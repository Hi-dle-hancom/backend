...
class FastVLLMClient:
    ...
    async def stream_generate(self, request: CodeGenerationRequest, user_id: str) -> AsyncGenerator[str, None]:
        ...
        # vLLM 요청 데이터 준비 - vLLM 서버 스키마에 맞게 수정
        mapped_model = self.model_mapping.get(request.model_type, VLLMModelType.PROMPT)
        vllm_request = {
            "user_id": hash(user_id) % 1000000,  # 사용자 ID 해시
            "prompt": request.prompt,
            "model_type": mapped_model.value,
            "user_select_options": {},  # 빈 딕셔너리로 초기화
        ...
        logger.log_system_event(
            ...
            "started",
            {
                ...
                "original_model_type": request.model_type.value,
                "mapped_vllm_model": mapped_model.value
            }
        )
